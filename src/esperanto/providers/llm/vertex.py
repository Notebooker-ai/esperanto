...
class VertexLanguageModel(LanguageModel):
    ...
    def _format_messages(self, messages: List[Dict[str, Any]]) -> tuple:
        ...
        for msg in messages:
            ...
            if role == "system":
                ...
                parts = self._convert_content_parts_to_vertex(content)
                formatted.append({
                    "role": "user",
                    "parts": parts,
                })
    ...
    def _convert_content_parts_to_vertex(
        self, content: Any
    ) -> List[Dict[str, Any]]:
        """Convert message content from OpenAI format to Vertex AI parts format.

        Args:
            content: Either a string or a list of content parts in OpenAI format.

        Returns:
            List of Vertex AI-format parts.
        """
